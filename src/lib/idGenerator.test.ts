...
describe('idGenerator', () => {
  it('generates date-stamped, 4-digit-suffixed ids for every entity type', () => {
    expect(generateOrderId()).toMatch(/^PPR-ORD-\d{8}-\d{4}$/);
    expect(generateReservationId()).toMatch(/^PPR-RES-\d{8}-\d{4}$/);
    expect(generateEmployeeId()).toMatch(/^PPR-EMP-\d{8}-\d{4}$/);
    expect(generateInventoryId()).toMatch(/^PPR-STK-\d{8}-\d{4}$/);
    expect(generateMenuItemId()).toMatch(/^PPR-DSH-\d{8}-\d{4}$/);
  });
  ...
  it('generates a fresh invoice number when no order id is given', () => {
    expect(generateInvoiceNo()).toMatch(/^PPR-INV-\d{8}-\d{4}$/);
  });
  ...
  it('derives a category id from its slug, or generates one without a slug', () => {
    ...
    expect(generateCategoryId()).toMatch(/^CAT-\d{8}-\d{4}$/);
  });
});
